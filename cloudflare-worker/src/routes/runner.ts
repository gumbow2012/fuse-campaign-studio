...
import { getProject, getTemplate, supabaseFetch, updateProjectStatus } from "../supabase";
...
export async function handleProjectStatus(request: Request, env: Env, projectId: string) {
...
async function callFalNanoBanana(
  ...
  if (directRes.ok) {
    const data = await directRes.json<any>();
  ...
  for (let i = 0; i < 120; i++) {
    ...
    if (status.status === "COMPLETED") {
      ...
      const result = await resultRes.json<any>();
...
async function generateKlingJwt(accessKey: string, secretKey: string) {
...
async function callKling(
  ...
  if (!taskId) {
    throw new Error(`Kling: no task_id — ${submitData.message || "unknown error"}`);
  }
...
async function uploadToR2(
  env: Env,
  projectId: string,
  item: { type: "image" | "video"; url: string; label?: string }
) {
  ...
  try {
    ...
    if (res.ok && res.body) {
      ...
      await env.FUSE_ASSETS.put(key, res.body, {
...
async function executeNode(
  ...
  projectInputs: Record<string, any>,
  progressBase: number,
  ...
  if (type === "input" || type === "file") {
    const inputKey = node.data?.key || node.data?.input_key || node.id;
    const value =
      projectInputs[inputKey] ||
      projectInputs[`${inputKey}_key`] ||
      Object.values(projectInputs).find(
        (v) => typeof v === "string" && (v.startsWith("http") || v.startsWith("uploads/"))
      ) ||
      "";

    const resolved = resolveImageUrl(String(value));
    await onProgress(progressBase, `Input "${inputKey}" resolved`);
    return resolved;
  }

  if (type === "prompt" || type === "text") {
    const prompt = node.data?.prompt || node.data?.text || node.data?.value || "";
    await onProgress(progressBase, `Prompt loaded (${String(prompt).length} chars)`);
    return prompt;
  }
  ...
  if (
    ...
    if (node.data?.prompt) prompt = node.data.prompt;
    ...
    return callFalNanoBanana(env, imageUrl, prompt, async (msg) => {
  ...
  if (
    ...
    return callKling(env, imageUrl, prompt, async (msg) => {
      await onProgress(progressBase + 5, msg);
    });
  }
...
async function runFlatPipeline(
  ...
  for (const key of [
    ...
    if (typeof projectInputs[key] === "string" && projectInputs[key].trim()) {
      imageUrl = resolveImageUrl(projectInputs[key]);
      break;
    }
  }
  ...
  const prompt = template.ai_prompt || "professional product photo, high quality";
  ...
  if (outputType !== "video") {
    ...
    const item = await uploadToR2(env, projectId, {
      type: "image",
      url: editedImageUrl,
      label: "Edited Image",
    });
  ...
  const imgItem = await uploadToR2(env, projectId, {
    type: "image",
    url: editedImageUrl,
    label: "Edited Image",
  });

  const vidItem = await uploadToR2(env, projectId, {
    type: "video",
    url: videoUrl,
...
async function runGraphPipeline(
  ...
  template: any,
  projectInputs: Record<string, any>,
  rawJson: { nodes: GraphNode[]; edges: GraphEdge[] }
) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    await setProgress(
      ...
      projectId,
      progressBase,
      `Executing node ${i + 1}/${sorted.length}: ${node.type} (${node.id.slice(0, 8)})`
    );
    ...
    const output = await executeNode(
      ...
      progressBase,
      async (pct, msg) => {
        await setProgress(env, projectId, pct, msg);
      }
    );
  ...
  const finalItems: Array<{ type: "image" | "video"; url: string; label?: string }> = [];
  ...
  for (const outNode of outputNodes) {
    ...
    for (const u of urls) {
      if (typeof u === "string" && u.startsWith("http")) {
        ...
        const label = outNode.data?.label || mediaType;
...
async function tryLoadTemplateGraphFromR2(env: Env, template: any) {
  const possibleKeys = [
    template?.template_key,
    template?.slug ? `${template.slug}.json` : null,
    template?.name ? `${String(template.name).toLowerCase().replace(/\s+/g, "-")}.json` : null,
    template?.id ? `${template.id}.json` : null,
...
async function runJob(env: Env, projectId: string) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const projectInputs = project.inputs || {};

      let rawJson = template.raw_json;
      if (!rawJson?.nodes) {
        rawJson = await tryLoadTemplateGraphFromR2(env, template);
      }
      ...
      if (hasGraph) {
        ...
        await runGraphPipeline(env, projectId, template, projectInputs, rawJson);
      } else {
