          ...
          <div className="space-y-6">
            <section className="rounded-[2rem] border border-white/10 bg-slate-950/75 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur-xl">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xl font-semibold capitalize text-white">
                    {profile?.plan && ["active", "trialing"].includes(profile.subscription_status ?? "")
                      ? profile.plan
                      : "No active plan"}
                  </p>
                  {profile?.subscription_status && ["active", "trialing"].includes(profile.subscription_status) ? (
                    <p className="mt-1 text-xs text-slate-400 capitalize">{profile.subscription_status}</p>
                  ) : null}
                </div>
              </div>
              ...
              ["active", "trialing"].includes(profile.subscription_status ?? "") ? (
                ...
                <p className="mt-5 text-sm text-slate-400">
                  Credits come with a membership or one-time top-ups.
                </p>
              )}
